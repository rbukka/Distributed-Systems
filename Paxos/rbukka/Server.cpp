...
time_t timer_start_vcproof;

void expire_vc_proof_timer()
{
	time_t current_time;
	time(&current_time);

	if(difftime(current_time,timer_start_vcproof) > VCProof_Timer)
		Send_VC_Proof();
	
	timer_start_vcproof = current_time;
}

void expire_update_timer()
{
	time_t current_time;
	time(&current_time);
	...
	for(map<int,time_t>::iterator it = update_time_for_timer.begin(); it!=update_time_for_timer.end(); it++)
	{
																	...
																	if(DEBUG)
																	{
																		fprintf(stderr, "Start time for client %ld\n", start);
																		fflush(NULL);
																	}

																	if(difftime(current_time,start) > Update_Timer)
																	{
																		kprintf("Update TImer is expired");
																		update_time_for_timer[client_id] = current_time;	
																		if(my_state == REG_NONLEADER)
																		{
																			kprintf("Update timer is expired and state is REG_NONLEADER");
																			//Send to Leader
																			...
																			if(Pending_Updates.find(client_id) != Pending_Updates.end())
																			{
...
int main(int argc, char**argv)
{
			...
			if(hostfile_stream.is_open())
			{
				...
				{		
						...
						{							
							...
							kprintf(host_to_id[hostname].c_str());	
						}
				}			
    	...
    	if (p == NULL)  {
			...
			//start vc proof timer
			time(&timer_start_vcproof);
			...
			for(;;)
			{
						...
						long diffMS = ((end.tv_sec * 1000000 + end.tv_usec)- (curr.tv_sec * 1000000 + curr.tv_usec));
						...
						if(diffMS > Progress_Timer)
						{
							tv.tv_sec = 0;
							tv.tv_usec = 0;
							kprintf("Progress Timer Expired 0");
						
						}
						else
						{
							diffMS = Progress_Timer - diffMS;
							tv.tv_sec = diffMS/ 1000000 ;
							tv.tv_usec = (diffMS - tv.tv_sec * 1000000);  
						}
					
						if(DEBUG)
						{
					  	fprintf(stderr,"Select Time: Sec: %ld and micro: %ld\n", tv.tv_sec, tv.tv_usec);
							fflush(NULL);
						}
				...
				//		kprintf("usec",tv.tv_usec);
						...
						kprintf("Return value of select ",ret);
						...
						else if(ret)
						{
							for(iter=0;iter<=fdmax;iter++)
							{
									if(FD_ISSET(iter,&read_fds))
									{
										...
										if(iter == client_sockfd)
										{
												...
												else
												{
													...
													kprintf("Connection received from",string(inet_ntoa(cliaddr.sin_addr)).c_str());
													kprintf("Created new fd",newfd);

													string currentIP = 	string(inet_ntoa(cliaddr.sin_addr));
													client_id_to_new_fd[currentIP] = newfd;
										...
										{
												kprintf("Inside paxos_sockfd");
												int n;
												
												n = recvfrom(paxos_sockfd,mesg,MAX_SIZE,MSG_DONTWAIT,(struct sockaddr *)&cliaddr,&lensock);
												
												if(n<0)
													kprintf("Error",errno);

												kprintf("recieved bytes",n);
					                  ...
					                  type = ntohl(type);
														...
														{
															kprintf("Received view change ",TYPE_VIEW_CHANGE);		
															View_Change* vc = (View_Change*)malloc(sizeof(View_Change));
															memcpy(vc,mesg,sizeof(View_Change));
															network_host(vc);

															if(!(Conflict((char *)vc,TYPE_VIEW_CHANGE)))
															{
																//Network byte order
																kprintf("vc->attempted ",vc->attempted);
																kprintf("Last_Attempted " , Last_Attempted);

																if(vc->attempted > Last_Attempted && !(progressTimer))
																{
																...
																if(vc->attempted == Last_Attempted)
																{
																	...
																	{
																		kprintf("Inside Preinnstall Ready");
														...
														{
															kprintf("Received vc proof ",TYPE_VC_PROOF);	
														...
														{
															kprintf("Received proposal ",TYPE_PROPOSAL);
															...
															if(Conflict((char *)c_p,TYPE_PROPOSAL) == false)
															{
																...
																for(map<string,string>::iterator it=host_to_id.begin(); it != host_to_id.end(); it++)
																{
																	...
																	kprintf("Sent Accept message ",it->second.c_str());
																}
																network_host(c_a);
															}
															else
																kprintf("Conflict proposal is true");
														}

														if(type == TYPE_ACCEPT)
														{
															kprintf("Received accept ",TYPE_ACCEPT);
														...
														if(type == TYPE_PREPARE)
														{
															kprintf("Received prepare ",TYPE_PREPARE);
															...
															if(Conflict((char *)c_pr,TYPE_PREPARE) == false)
															{
																...
																if(my_state == LEADER_ELECTION)
																{
																	...
																	char *to_be_sent = (char *)malloc(size);
																	...
																	int rval = sendto(paxos_sockfd,to_be_sent,size,0,(struct sockaddr *)&cliaddr,sizeof(cliaddr));								
																	kprintf("Sent prepare ok message ",leader_ip.c_str());																
																	network_host(c_pok);
																	free(to_be_sent);
																	to_be_sent = NULL;
																...
																else
																{
																	...
																	if(Prepare_oks.find(My_server_id) != Prepare_oks.end())
																	{
																		Prepare_OK* c_pok = Prepare_oks[My_server_id];
																	
																		kprintf("After c_pok",c_pok->total_globally_ordered_updates);	
																		...
																		for(map<string,int>::iterator it = hostname_to_id.begin(); it != hostname_to_id.end(); it++)
																		{
																			kprintf("leader ",leader);
																			kprintf("it->second ",it->second);
															...
															{
																kprintf("Conflicting Prepare",c_pr->view);
																kprintf("last attempted", Last_Attempted);
															}
														}
														...
														{
															kprintf("***************");
															kprintf("Recieved prepare ok ", TYPE_PREPARE_OK);
									...
									{
													if((n = recvfrom(iter,mesg,MAX_SIZE,MSG_DONTWAIT,(struct sockaddr *)&cliaddr,&lensock)) > 0)
													{
														uint32_t* recv_type = (uint32_t*)malloc(sizeof(uint32_t));
					                  memcpy(recv_type,mesg,sizeof(uint32_t));
					                  int type = *recv_type;
					                  type = ntohl(type);
															...
															client_id_fd[c_up->client_id] = iter;
															...
															if(DEBUG)
															{
																fprintf(stderr, "clientID: %d update: %d start time:%ld\n %s",c_up->client_id, c_up->update, update_time_for_timer[c_up->client_id],string(client_id_to_ip[c_up->client_id]).c_str()); 
																fflush(NULL);
															}
						...
						{
								expire_update_timer();
								kprintf("Progress Timer expired");
								progressTimer = false;
								Leader_Election();
								//Send_VC_Proof();
						}
