...
//Map to store client IP to new fd
map<int,int> client_id_fd;


//Master fd
fd_set master;

//Map to hold Client IP to new fd
map<string,int> client_id_to_new_fd;
...
long Progress_Timer = 2000000;
double Update_Timer	 = 1;
bool progressTimer  =  true;
double VCProof_Timer = 1;
...
bool Conflict(char* mesg,int type)
{
	...
	switch(type)
	{
		case TYPE_VIEW_CHANGE:
		{
				...
				kprintf("server id ",server_id);
				kprintf("attempted ",attempted);
				kprintf("Last Installed ", Last_Installed);
				kprintf("progressTimer ", progressTimer);
		...
		case TYPE_PREPARE_OK:
		{
			...
			kprintf("prepare ok server id",prepare_ok->server_id);
		...
		case TYPE_PROPOSAL:
		{
			...
			kprintf("conflict proposal server id", proposal->server_id);
			kprintf("conflict proposal my state",my_state);
			kprintf("conflict view",proposal->view);
			kprintf("conflict Last installed",Last_Installed);
		...
		case TYPE_ACCEPT:
		{
			...
			kprintf("Accept view ",accept->view);
			kprintf("Last_Installed ", Last_Installed);
			kprintf("My_server_id ",My_server_id);
			kprintf("accept->server_id",accept->server_id);
			...
			if(global_history.find(accept->seq) != global_history.end() && (global_history[accept->seq]))
			{
				...
				kprintf("Found global history", accept->seq);
	...
	switch(type)
	{
		...
		{	
			...
			if(VC.find(v_change->server_id) == VC.end())
				...
				kprintf("View Change message exists for server ",v_change->server_id);
			break;
		}
		case TYPE_PREPARE:
		{
		...
		{
			Proposal* proposal = (Proposal*)mesg;
			kprintf("In update_data_structures proposal",proposal->seq);
			kprintf("Searching global history");
			
	
			if(global_history.find(proposal->seq) == global_history.end() || !(global_history[proposal->seq]))
			{
				global_history[proposal->seq] = new Global_Slot;
				global_history[proposal->seq]->latest_proposal_accepted = NULL;
				global_history[proposal->seq]->g_ordered_update = NULL;
				global_history[proposal->seq]->Accepts.clear();
				kprintf("Created");
			}
		
			Globally_Ordered_Update* msg = global_history[proposal->seq]->g_ordered_update;
			Proposal* propmsg = global_history[proposal->seq]->latest_proposal_accepted;
			kprintf("Got prop msg");
		...
		case TYPE_ACCEPT:
		{
			...
			if(global_history.find(accept->seq) == global_history.end() && !(global_history[accept->seq])) 
			{
				global_history[accept->seq] = new Global_Slot;
				global_history[accept->seq]->latest_proposal_accepted = NULL;
				global_history[accept->seq]->g_ordered_update = NULL;
				global_history[accept->seq]->Accepts.clear();
			}

			Globally_Ordered_Update* globMsg = global_history[accept->seq]->g_ordered_update;
			...
			if(global_history[accept->seq]->Accepts.size() >= N/2)
			{
				kprintf("Already has N/2 accepts");	
				return;
			}

			bool isAcceptPresent = false;
			if(global_history[accept->seq]->Accepts.find(accept->server_id) != global_history[accept->seq]->Accepts.end())
			{
				Accept* accmsg = global_history[accept->seq]->Accepts[accept->server_id];
				...
				{
					kprintf("Accept not found some error in update");
				...
				if(!isAcceptPresent)
				{
					kprintf("Storing accept in map" , accept->seq);
					kprintf("Global history",global_history[accept->seq]->Accepts.size());
					global_history[accept->seq]->Accepts[accept->server_id] = accept;
					kprintf("Stored accept in map" , accept->seq);
				}
		...
		case TYPE_GLOBALLY_ORDERED_UPDATE:
		{
			...
			{
				global_history[global->seq] = new Global_Slot;
				global_history[global->seq]->latest_proposal_accepted = NULL;
				global_history[global->seq]->g_ordered_update = NULL;
				global_history[global->seq]->Accepts.clear();
			}
...
{	
	kprintf("In View_Prepared_Ready",view);
	int count=0;
	for(map<int,Prepare_OK*>::iterator it=Prepare_oks.begin();it!=Prepare_oks.end();it++)
	{
		Prepare_OK* p = (Prepare_OK*)(it->second);

		kprintf("In Prepare OK of view prepared ready", p->server_id);

		if(p->view == view)
			count++;
	}

	kprintf("Count of view", count);
...
{
	kprintf("In Preinstall_Ready",view);
	int count=0;
...
Prepare* Construct_Prepare(int view,int aru)
{
...
Globally_Ordered_Update* Construct_Globally_Ordered_Update(int seq,int server_id,Client_Update update)
{
	...
	g_update->type = TYPE_GLOBALLY_ORDERED_UPDATE;
...
Prepare_OK* Construct_Prepare_OK(int view,vector<Proposal*> *proposal_list,vector<Globally_Ordered_Update*> *global_list)
{
...
void Shift_to_Prepare_Phase()
{
	...
	kprintf("Sending Prepare type",c_p->type);
	...
	for(map<string,string>::iterator it=host_to_id.begin(); it != host_to_id.end(); it++)
	{
		...
		kprintf("Sent prepare message ",it->second.c_str());
	}
	network_host(c_p);
}

View_Change* Construct_VC(int view)
{
...
{
	kprintf("In Globally_Ordered_Ready", global_history.size());
	...
	if(global_history.find(seq) != global_history.end() && global_history[seq])
	{
		if(global_history[seq]->latest_proposal_accepted != NULL)
		{
				...
				for(map<int,Accept*>::iterator it=accepts.begin(); it!=accepts.end(); it++)
				{
					...
					kprintf("In Globally Ordered Readey", view);

					if(view_count.find(view) == view_count.end())
						view_count[view] = 1;
					else
						view_count[view] = view_count[view] + 1;
				}

				for(map<int,int>:: iterator it = view_count.begin(); it!= view_count.end(); it++)
				{	
					kprintf("Globally ordered Ready: view count ",it->second);
					if(it->second >= N/2)
						return true;
				}
...
void Send_Proposal()
{
	...
	if(!updateFound)
	{
		...
		{
			kprintf("Getting the front of update queue");
	...
	for(map<string,string>::iterator it=host_to_id.begin(); it != host_to_id.end(); it++)
	{
		...
		kprintf("Sent proposal message ",it->second.c_str());
	}
			...
			if(global_history.find(i) != global_history.end() && global_history[i])
			{
				if(global_history[i]->g_ordered_update != NULL)
				{
					...
					//Upon executing a client update
						...
						fprintf(stderr,"%d: Executed update %d from client %d with seq %d and view %d\n",My_server_id,cli_update.update,cli_update.client_id,seq,Last_Installed);
						fflush(NULL);
						...
						if(cli_update.server_id == My_server_id)
						{
							...
							if(client_id_to_ip.find(cli_update.client_id) != client_id_to_ip.end())
							{
								...
								//int newfd = client_id_to_new_fd[ip];
								int newfd=client_id_fd[cli_update.client_id];

								kprintf("Client fd is",newfd);

								int rval = send(newfd, "Hello, world!", 13, 0);
...
void Client_Update_Handler(Client_Update* U)
{
	...
	if(my_state == LEADER_ELECTION)
	{
	  kprintf("In Leader Election");
		if(U->server_id != My_server_id)
			return;
		if(Enqueue_Update(U))
			Add_to_Pending_Updates(U);
	}
	else if(my_state == REG_NONLEADER)
	{
	  kprintf("In REG NON LEADER ");
		if(U->server_id == My_server_id)
			Add_to_Pending_Updates(U);
		...
		kprintf("Sent client update message ",leader_ip.c_str());																
		network_host(U);									

	}
	else if(my_state == REG_LEADER)
	{

	 kprintf("In REG LEADER");
		if(Enqueue_Update(U))
		{
			if(U->server_id == My_server_id)
			{
...
{
	kprintf("In Shift_to_Leader_Election", view);
	...
	for(map<string,string>::iterator it=host_to_id.begin(); it != host_to_id.end(); it++)
	{
		...
		kprintf("Sent view change message ",it->second.c_str());
	}
...
void Send_VC_Proof()
{
	...
	for(map<string,string>::iterator it=host_to_id.begin(); it != host_to_id.end(); it++)
	{
		...
		kprintf("Sent vc proof message ",it->second.c_str());
	}	
	network_host(vc_p);
