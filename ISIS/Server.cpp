	...
	for(map<Key,DM*>:: iterator it= (*delivery_queue).begin(); it!= (*delivery_queue).end();it++)
	{
	  kprintf("In for loop delivery_queue");
		if(it->second->deliverable == true)
		{
			...
			printf("%d : Processed message %d from sender %d with seq %d \n",mypid+1,(it->second)->data_msg->msg_id,(it->second)->data_msg->sender+1,(it->second)->seq_num);
			fflush(stdout);
...
int main(int argc, char**argv)
{
		  ...
		  //count of messages to be sent
			...
			{
				printf("Usage: \n");
				printf("proj2 -p port -h hostfile -c count \n");
				exit(EXIT_FAILURE);
			}
		  ...
		  for(i=0;i<argc;i++)
		  {
					 ...
					 if(arg == "-c")
					 {
					 			if(argv[i+1])
								{
									...
									if(count < 0)
									{
										printf("Number of messages to be multicast must be greater than zero\n");
										exit(EXIT_FAILURE);
									}
								}
								else
								{
									printf("Please specify number of messages to be multicast\n");
									exit(EXIT_FAILURE);
								}
					 }
			...
			{
				printf("Unable to read host file \n");
				exit(EXIT_FAILURE);
			}
